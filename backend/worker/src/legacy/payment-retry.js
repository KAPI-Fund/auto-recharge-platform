...
async function executePaymentWithRetry(page, options) {
	...
	const baseOwnerKey = taskId
	    ? `task_${taskId}`
	    : (cdkCode ? `payment_${cdkCode}` : 'payment_debug');
        ...
        try {
            ...
            if (paymentResult.success) {
                ...
                await store.createBillingRecord({
					...
					pool_id: card.pool_id || process.env.POOL_ID || '',
					task_id: taskId,
                    amount: billedAmount,
                    currency: billedCurrency,
                    plan_type: planType || 'plus',
            ...
            if (declined) {
				...
				const settlement = await settleAttempt(card, { failureCode: 'card_declined', failureMessage: lastError });
				cardHandled = settlement.accepted;
