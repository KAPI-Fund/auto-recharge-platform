...
async function runGptApiWorker({ task, token, session, cdk, planType }) {
    ...
    try {
		...
		if (finalStatus === 'success') {
            ...
            const settlement = await settleOneTimeCard(store, reservedCard);
            if (!settlement.ok) {
                logTask(jobKey, `银行卡收尾未完全成功: ${settlement.error?.message || 'Provider 销卡待重试'}`, 'warn');
            }
            notifyTaskOutcome({ event: 'success', email: accountEmail, cdk, jobKey, message: finalMessage });
        } else {
            // A card passed to the upstream payment API has made a real
            // payment attempt. Preserve the same failure attribution used by
            // the legacy Checkout flow before releasing/cancelling it.
            const settlement = await settleOneTimeCard(store, reservedCard, {
                failureCode: 'gpt_api_payment_failed',
        ...
        const reservedCardId = String(reservedCard?.id || '').trim();
		if (reservedCardId) {
            const settlement = await settleOneTimeCard(store, reservedCard, {
                failureCode: 'gpt_api_payment_failed',
            ...
            if (!settlement.ok) {
                logTask(jobKey, `异常任务的银行卡收尾未完全成功: ${settlement.error?.message || 'Provider 销卡待重试'}`, 'warn');
            }
        }
