...
export function RechargeView({ initialTab = "redeem", initialPlanCode = "" }: { initialTab?: RechargeTab; initialPlanCode?: string }) {
  ...
  async function handlePurchase() {
    ...
    if (planIsSoldOut(plan)) return setNotice({ tone: "danger", text: "已售罄，补货中" });
  ...
  function choosePurchasePlan(planCode: string) {
    ...
    if (selected && planIsSoldOut(selected)) {
      setNotice({ tone: "danger", text: "已售罄，补货中" });
      return;
    }
  ...
  const selectedPurchasePlanSoldOut = Boolean(selectedPurchasePlan && planIsSoldOut(selectedPurchasePlan));
  ...
  return (
    <main className="recharge-page">
      ...
      <div className="page-shell">
        ...
        {activeTab !== "purchase" ? <section className="plan-showcase" aria-label="可购买套餐">{plans.map((plan) => { const soldOut = planIsSoldOut(plan); return <button key={plan.code} type="button" className={cn("plan-card", plan.code === "pro_5x" && "highlight", soldOut && "sold-out")} disabled={taskActive || soldOut} onClick={() => choosePurchasePlan(plan.code)}><div className="plan-card-head"><h3>{planTitle(plan)}</h3><span className={cn("plan-tag", plan.code !== "plus" && "pro")}>{planTag(plan)}</span></div><div className="plan-card-price">{planPrice(plan)}</div><p>{planDescription(plan)}</p><span className={cn("plan-card-stock", soldOut && "sold-out")}>{planStockLabel(plan)}</span><span className="plan-card-action">{soldOut ? "已售罄，补货中" : "选择套餐"} <span aria-hidden="true">{soldOut ? "" : "→"}</span></span></button>; })}</section> : null}
